...
ColorPalette GetPalette(Entity *entity);
void GetContent(Entity *entity, char content[5]);
void Init();
void DrawLabels();
void *Update();
...
int main() {
  ...
  Rectangle frame = (Rectangle){MARGIN - PADDING, MARGIN - PADDING, GUI_MAP_SIZE + PADDING * 2,
...
void Init() {
  PALLETES.green = (struct ColorPalette){.background = Fade(GREEN, .8), .border = DARKGREEN};
  PALLETES.blue = (struct ColorPalette){.background = Fade(BLUE, .5), .border = DARKBLUE};
  PALLETES.yellow = (struct ColorPalette){.background = Fade(YELLOW, .8), .border = ORANGE};
  PALLETES.red = (struct ColorPalette){.background = Fade(RED, .5), .border = MAROON};
  PALLETES.blank = (struct ColorPalette){.background = BLANK, .border = Fade(DARKGRAY, .9)};
...
void *Update() {
  ...
  rd_kafka_t *consumer = createKafkaUser(&kafka, RD_KAFKA_CONSUMER, NULL);
  ...
  do {
    ...
    for (int i = 0; response.map[i] != 0; i++) {
      Entity entity;
      deserializeUser(&entity, response.map[i]);
      if ((entity.type == ENTITY_CUSTOMER && entity.status == STATUS_CUSTOMER_IN_TAXI) ||
          (entity.type == ENTITY_TAXI && entity.status == STATUS_TAXI_DISCONNECTED))
        continue;
      colors[entity.coord.x][entity.coord.y] = GetPalette(&entity);
      GetContent(&entity, content[entity.coord.x][entity.coord.y]);
    }
...
ColorPalette GetPalette(Entity *entity) {
  switch (entity->type) {
  case ENTITY_TAXI:
    if (entity->status == STATUS_TAXI_CANT_MOVE || entity->status == STATUS_TAXI_STOPPED)
      return PALLETES.red;
...
void GetContent(Entity *entity, char content[5]) {
  ...
  if (entity->status == STATUS_TAXI_CANT_MOVE) {
    content[currentIndex] = '!';
    currentIndex++;
  }

  if (entity->type == ENTITY_TAXI) {
    sprintf(content + currentIndex, "%i", entity->id);
    if (entity->id >= 10)
      currentIndex += 2;
    else
      currentIndex++;
  } else {
    content[currentIndex] = entity->id;
    currentIndex++;
  }

  if (entity->type == ENTITY_TAXI && entity->carryingCustomer) {
    content[currentIndex] = entity->obj;
    currentIndex++;
  }
